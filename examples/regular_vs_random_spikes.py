...
for i in range(NO_EXPERIMENTS):
    try:
        ...
        base_freq = 180
        #freq = base_freq + (i // 2) * 50
        freq = base_freq
        generate_spikes(uc, freq, duration*1000, TARGET_NEURON, type="exc", regular = (i % 2))
...
for i in range(NO_EXPERIMENTS):
    ...
    try:
        ...
        if min_t is None or max_t == min_t:
            ...
            frequency = n / (max_t - min_t) * 1_000_000
            if (i + 1) % 2 != 0:  # odd experiment index → fixed/regular spiking
                fixed_freqs.append((i + 1, frequency))
                print(f"Experiment {i+1} [fixed]:  {frequency:.2f} Hz")
            else:                  # even experiment index → random spiking
                random_freqs.append((i + 1, frequency))
                print(f"Experiment {i+1} [random]: {frequency:.2f} Hz")

    except Exception as e:
        uc.close_connection()
        raise Exception(f"An error occurred in experiment {i+1}") from e

print("\n--- Fixed (regular) spiking experiments ---")
for exp, freq in fixed_freqs:
    print(f"  Experiment {exp}: {freq:.2f} Hz")

print("\n--- Random spiking experiments ---")
for exp, freq in random_freqs:
    print(f"  Experiment {exp}: {freq:.2f} Hz")
