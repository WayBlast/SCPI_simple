# Run from root with: python -m examples.inh_vs_exc
import time
import sys
import csv
import pyvisa
...
for i in range(NO_EXPERIMENTS):
    try:
        ...
        freq = 3 if i < 2 else 20
        spike_type = "inh" if (i == 1 or i == 3) else "exc"

        generate_spikes(uc, freq, duration*1000, TARGET_NEURON, type=spike_type)
...
for i in range(NO_EXPERIMENTS):
    try:
        ...
        time_ax = data[:, 0]
        voltage = data[:, 1]
        plt.plot(time_ax * 1000, voltage, color=colors[i % len(colors)], label=f"Experiment {i+1}", alpha=0.65)
